...
class Instrument:
    ...
    def _process(self) -> None:
        ...
        # if re.findall(self._contract_type) != []:
        #     self._expiration_date = re.findall(r'[0-9]+', self._contract_type)[0]
        #     self._contract_type = re.sub(r'[0-9]', '', self._contract_type)
        # self._contract_type, self._expiration_date = Instrument.format_quarter_contract(self._contract_type)
    ...
    def get_contract_expiration_date(self, date:str) -> None:
        ...
        if self._contract_type == InstrumentType.FutureThisQuarter:
            if dt <= ( expiration_dt - main_contract_change_duration ):
                ...
                self._change_contract_dt = expiration_dt - main_contract_change_duration
        elif self._contract_type == InstrumentType.FutureNextQuarter:
            if dt <= ( expiration_dt - main_contract_change_duration ):
                self._expiration_date = next_expiration_date
                self._expiration_dt = next_expiration_dt.astimezone(timezone('Asia/Shanghai'))
                self._change_contract_dt = expiration_dt - main_contract_change_duration
    ...
    def get_instrument_type_class(exchange, symbol:str, contract_type:str) -> str:
        if contract_type == InstrumentType.Spot:
            return InstrumentTypeClass.Spot
        elif contract_type == InstrumentType.FutureLTFX or exchange in Instrument.FORWARD_FUTURES_EXCHANGES:
    ...
    def format_instrument_id(instrumentId:str) -> str:
        ...
        for key, value in InstrumentType.__dict__.items():
